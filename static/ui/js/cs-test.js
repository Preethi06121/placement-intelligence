  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    api("/api/me")
      .then(function (me) {
        ...
        return api("/api/cs_test");
      })
      .then(function (data) {
        var qs = data.questions || [];
        var html =
          '<div class="card"><h1>CS knowledge test</h1><form id="form-cs">';
        ...
        document.getElementById("form-cs").addEventListener("submit", function (e) {
          e.preventDefault();
          ...
          err.hidden = true;
          api("/api/cs_test", { method: "POST", body: { answers: answers } })
            .then(function (res) {
              showResults(main, res, "CS result");
            })
            .catch(function (x) {
      ...
      .catch(function () {
        if (main.innerHTML.indexOf("form-cs") === -1 && main.textContent.indexOf("Loading") !== -1) {
          main.innerHTML = '<div class="card"><p class="error">Could not load test.</p></div>';
        }
      });
  });
})();
